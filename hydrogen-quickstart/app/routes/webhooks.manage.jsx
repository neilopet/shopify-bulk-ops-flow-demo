...
} from '~/lib/webhooks/webhook-manager.server';
...
export async function loader({context}) {
  ...
  try {
    ...
    return Response.json({
      webhook: response?.webhookSubscription,
      error: null,
      environment,
    });
  } catch (error) {
  ...
  return (
    <div style={{padding: '2rem', fontFamily: 'system-ui, sans-serif'}}>
      ...
      <section style={{marginTop: '2rem'}}>
        ...
        >
          <p>
            <strong>Environment:</strong> {environment.nodeEnv}
          </p>
          <p>
            <strong>Store Domain:</strong> {environment.storeDomain}
          </p>
          <p>
            <strong>Webhook Base URL:</strong> {environment.webhookBaseUrl}
          </p>
          <p>
            <strong>Local Webhook URL:</strong> {environment.localWebhookUrl}
          </p>
        </div>
      </section>
      ...
      <section style={{marginTop: '2rem'}}>
        ...
        <ul>
          <li>
            Click "Register/Update Webhook" to ensure the webhook is registered
            with the current deployment URL
          </li>
          <li>
            The webhook will automatically update its callback URL when the app
            URL changes
          </li>
          <li>
            For Hydrogen preview deployments, set WEBHOOK_BASE_URL to your
            preview URL
          </li>
        </ul>
      </section>
