...
const client = new GraphQLClient(
  `https://${process.env.PUBLIC_STORE_DOMAIN}/admin/api/2025-07/graphql.json`,
  {
    headers: {
...
async function getManagedWebhook() {
  ...
  try {
    const {webhookSubscription} = await client.request(GET_WEBHOOK_QUERY, {
      id: webhookId,
    });
    if (
      webhookSubscription &&
      webhookSubscription.topic === 'BULK_OPERATIONS_FINISH'
    ) {
    ...
    console.log(
      `⚠️  Previously tracked webhook (${webhookId}) not found or invalid`,
    );
...
async function listWebhooks() {
  const {webhookSubscriptions} = await client.request(FIND_WEBHOOK_QUERY, {
    first: 100,
  });
  console.log('\nRegistered Webhooks:');
  console.log('==================');
...
async function createWebhook(url) {
  ...
  if (existingWebhook) {
    console.log(
      '⚠️  A managed webhook already exists. Use "update" command to change the URL.',
    );
  ...
  const {webhookSubscriptionCreate} = await client.request(
...
async function updateWebhook(url) {
  ...
  const {webhookSubscriptionUpdate} = await client.request(
...
async function deleteWebhook() {
  ...
  const {webhookSubscriptionDelete} = await client.request(
